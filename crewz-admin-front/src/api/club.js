...
export const getSubsidyList = (requestData) => {
  const uri = `/api/clubsubsidy?clubNo=${requestData.clubNo}&year=${requestData.year}`;
  const config = {};
  return request.get(uri, config);
};

export const confirmRemove = (requestData) => {
  const uri = `/api/club?clubNo=${requestData.clubNo}`;
  const config = {};
  return request.patch(uri, requestData, config);
};
