<template>
	<div class="dashboard p-4" style="height: 1130px">
		<!-- grid wrapper card -->
		<div
		...
		>
			...
			>
				...
				<div class="block p-2 w-full">
					<p
						class="font-semibold text-gray-900 dark:text-gray-200 text-xl"
					>
		...
		<div class="mt-4 lg:flex block lg:gap-2 relative">
			...
			>
				<div class="head p-5">
					<h1
						class="font-bold text-2xl text-gray-800 dark:text-gray-200"
					>
						분기별 동아리 추이
					</h1>
				</div>
...
<script setup>
	...
	const init = async () => {
		...
		for (let i = 0; i < clubCountByCategory.value.length; i++) {
			optionsCategoryDonut.value.labels.push(
				clubCountByCategory.value[i][0]
			);
			seriesDonut.value.push(clubCountByCategory.value[i][1]);
		}

		for (let i = 0; i < totalPriceByQuarter.value.length; i++) {
			optionsArea.value.xaxis.categories.push(
			...
			seriesSubsidyLine.value[0].data.push(
				totalPriceByQuarter.value[i][1]
			);
		}
